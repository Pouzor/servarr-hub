...
from app.models.models import ServerMetric, PlaybackSession
...
class MetricsService:
    ...
    def capture_metrics(db: Session) -> ServerMetric:
        ...
        try:
            ...
            active_sessions_count = db.query(PlaybackSession).filter(
                PlaybackSession.is_active == True
            ).count()
            
            # Compter les transcodages actifs
            active_transcoding_count = db.query(PlaybackSession).filter(
                PlaybackSession.is_active == True,
                PlaybackSession.transcoding_progress > 0
            ).count()
            ...
            logger.info(f"📊 Métriques capturées : CPU={cpu_percent}%, RAM={memory_usage_gb:.1f}GB, Sessions={active_sessions_count}")
    ...
    def cleanup_old_metrics(db: Session, keep_days: int = 7):
        ...
        try:
            ...
            deleted = db.query(ServerMetric).filter(
                ServerMetric.recorded_at < cutoff_date
            ).delete()
