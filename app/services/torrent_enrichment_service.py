...
class TorrentEnrichmentService:
    ...
    async def _get_qbt_connector(self):
        ...
        if self.qbt_connector is None:
            qbt_service = self.db.query(ServiceConfiguration).filter(
    ...
    async def enrich_item(self, item: LibraryItem) -> bool:
        ...
        try:
            ...
            logger.info(f"✅ Item {item.id} enrichi avec succès (ratio: {torrent_info.get('ratio')}, status: {torrent_info.get('status')})")
            return True
    ...
    async def enrich_all_items(self, limit: Optional[int] = None) -> dict:
        """
